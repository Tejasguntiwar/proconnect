...
export default function Dashboard() {
  ...
  const comments = Array.isArray(postState?.comments) ? postState.comments : [];
  ...
  const handleUpload = async () => {
    ...
    // if creation succeeded, refresh posts
    setPostContent('');
    setFileContent(null);
          ...
          <div className={styles.scrollComponent}>
            <div className={styles.createPostContainer}>
              <img className={styles.userProfile}
                src={`${BASEURL}/uploads/${authState.user?.userId?.profilePicture}`}
                alt="profile"
              />
              <textarea onChange={(e) => setPostContent(e.target.value)} value={postContent} className={styles.textAreaOfContent} name="postContent" placeholder="What's on your mind?"></textarea>
                      ...
                      <div className={styles.singleCardProfileContainer}>
                        ...
                        <div>
                          ...
                          <div className={styles.singleCardImage}>
                            <img src={`${BASEURL}/${post.media}`} alt="Post" />
                          </div>
            ...
            className={styles.commentsContainer}>
              ...
              className={styles.allCommentsContainer}>
                ...
                <div className={styles.commentsHeader}>
                  ...
                  <button
                    type="button"
                    className={styles.commentsHeaderCloseBtn}
                ...
                <div className={styles.commentsBody}>
                  {comments.length === 0 && (
                    <div className={styles.emptyCommentsState}>
                      ...
                      <p className={styles.emptyCommentsSubtitle}>Start the conversation by adding the first comment.</p>
                    </div>
                  )}

                  {comments.length !== 0 && (
                    <div className={styles.allCommentsContainer_comments}>
                      {comments.map((singleComment, index) => {
                        return (
                          <div className={styles.singleCommentContainer} key={singleComment._id || `${singleComment.userId?._id}-${index}`}>
                            <div className={styles.singleCommentContainer_profile}>
                              <img
                                className={styles.commentAvatar}
                                src={`${BASEURL}/uploads/${singleComment.userId.profilePicture}`}
                                alt="profile"
                              />
                              <div className={styles.singleCommentContainer_profileText}>
                                <p className={styles.commentAuthorName}>{singleComment.userId.name}</p>
                                <p className={styles.commentAuthorUsername}>@{singleComment.userId.username}</p>
                              </div>
                            </div>
                            <p className={styles.singleCommentContainer_commentBody}>{singleComment.body}</p>
                          </div>
                        )
                      })}
                    </div>
